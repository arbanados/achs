...
author: "Alejandro Bañados, Yasna Palmeiro, Luis Cifuentes & Pablo Cea"
output:
...
## 4.1 Table of Communes

```{r}
datafull[, .(
  total_accidents = sum(count, na.rm = TRUE),
  tmax_P1 = round(quantile(tmax, probs = 0.01, na.rm = TRUE), 1),
  tmax_P25 = round(quantile(tmax, probs = 0.25, na.rm = TRUE), 1),
  tmax_P50 = round(quantile(tmax, probs = 0.50, na.rm = TRUE), 1),
  tmax_P75 = round(quantile(tmax, probs = 0.75, na.rm = TRUE), 1),
  tmax_P99 = round(quantile(tmax, probs = 0.99, na.rm = TRUE), 1)
), by = Comuna][order(-total_accidents)]
```

## 4.3 Violin by macroregion
```{r}
# Prepare datasets for each macroregion
...
data_center[, MacroRegion := "Center"]
...
data_south[, MacroRegion := "South"]

# Combine all
combined_data <- rbindlist(list(data_north, data_center, data_south))

# Set desired factor level order
combined_data[, MacroRegion := factor(MacroRegion, levels = c("South","Center","North"))]


ggplot(combined_data, aes(y = MacroRegion, x = tmax, fill = MacroRegion)) +
  geom_violin(trim = FALSE, alpha = 0.6, color = NA) +
  geom_boxplot(width = 0.15, alpha = 0.9, color = "black",
               outlier.size = 1, outlier.alpha = 0.5) +
  scale_x_continuous(name = "Max Daily Temperature (°C)") +
  labs(
    y = "MacroRegion",
    title = "Distribution of Max Daily Temperatures"
  ) +
  theme_minimal() +
...
## 4.2 Model only warm season

```{r}
mod1=f_estimate_results(input_data = datafull, lag = 5, cen = 20, year_df = 8, output_name = "Main")
col <- "tomato4"
pch_symbol <- 16 
parold <- par(no.readonly = TRUE)
par(mar = c(4, 4, 1, 0.5), las = 1, mgp = c(2.5, 1, 0))
...
```{r}
subset <- datafull[month %in% c(10, 11, 12, 1, 2, 3)]
mod1=f_estimate_results(input_data = subset, lag = 5, cen = 20, year_df = 8, output_name = "Main")
col <- "tomato4"
pch_symbol <- 16 
parold <- par(no.readonly = TRUE)
par(mar = c(4, 4, 1, 0.5), las = 1, mgp = c(2.5, 1, 0))
...
```{r}
subset <- datafull[month %in% c(10, 11, 12, 1, 2, 3)]
mod1=f_estimate_results(input_data = subset, lag = 5, cen = 20, year_df = 4, output_name = "Main")
col <- "tomato4"
pch_symbol <- 16 
parold <- par(no.readonly = TRUE)
par(mar = c(4, 4, 1, 0.5), las = 1, mgp = c(2.5, 1, 0))
...
```{r}
# AM
data_am<- f_prepare_dlnm_data(
  accidents_data = rawdata,
  temperature_data = temp_data,
  filter_condition = "tipo_siniestro_actual_desc == 'Trabajo' & as.numeric(hora_accidente) < 43200",
  holidays_vec = holidays
)
cp_am <- f_estimate_results(input_data = data_am, output_name = "AM")

# PM
data_pm <- f_prepare_dlnm_data(
  accidents_data = rawdata,
  temperature_data = temp_data,
  filter_condition = "tipo_siniestro_actual_desc == 'Trabajo' & as.numeric(hora_accidente) > 43200",
  holidays_vec = holidays
)
cp_pm <- f_estimate_results(input_data = data_pm, output_name = "PM")

# Plot
f_plot2curves(cp_am[[1]], cp_pm[[1]], xlim = c(5, 40), legend = c("AM", "PM"), figurenumber = "extra")
```
